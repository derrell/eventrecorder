    ...
    members: {
      ...
      translate: function translate(script) {
        var lines = this._translate(script).split(/\n/).map(function (line) {
          return line.startsWith("(") ? "await ".concat(line, ";") : line;
      ...
      */
      ...
      cmd_await_property_match_json: function cmd_await_property_match_json(id, property, json) {
        ...
        var timeoutmsg = "Timeout waiting for ID(".concat(id, ").").concat(property, " to match /").concat(regExLiteral.replace(/\\/, "\\\\").replace(/"/g, "\\\""), "/.");
       ...
       * @return {String}
       */
      cmd_fire: function cmd_fire(id, event, json) {
        if (json) {
          if (!qx.lang.Type.isString(json)) {
            json = JSON.stringify(json);
          }
      ...
      * @return {String}
      */
      cmd_dbltap: function cmd_dbltap(id) {
        // doesn't work yet because it needs mouse data etc.
      ...
      cmd_await_selection: function cmd_await_selection(id, selectedId) {
        var timeoutmsg = "Timeout when waiting for selection of object '".concat(selectedId, "' on '").concat(id, "'.");
...
//# sourceMappingURL=Qooxdoo.js.map?dt=1556283835105